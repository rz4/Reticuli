...
```bash
pip install "git+https://github.com/rz4/reticuli"   # or from a checkout: pip install -e .
...
Reticuli is a **basin-compiler that compiles itself** — in five rungs, ordered
by interface volatility: the deeper the layer, the more stable its contract;
the outer layers face other parties — agents, humans — and churn. `ret pack`
declares a project as a record: its code the *free* outputs, its check the
*seed*, gated by running the check. Five records make up the repo, each
carrying everything below it and layering its own stratum on top, sealed by
[`scripts/selfrecord.py`](scripts/selfrecord.py):
...
| **reticuli** (surface) | `+ {cli,__main__}.py` | [`whole_check.py`](whole_check.py) | the human handshake → `VERIFIED` |

Each rung obtains the layers beneath it from its predecessor as `from` produce
steps — free code it *layers on*, not bytes it pins. So `ret verify .` holds,
...
threads it up through exchange, authoring, and agents, regrows the surface
over them, runs `whole_check.py`, and — because all the code is free and the
...
the invariant costs to regrow vs. what the volatile handshakes cost. A redo
with a *different* implementation of every rung still lands on the same five
roots (today, inner to outer: `af831de0…`, `63fa5e75…`, `f1168e37…`,
`b3d17a3e…`, whole `de97b1d8…`).
...
[`scripts/producer_openai.py`](scripts/producer_openai.py) is the same contract
for OpenAI models, and [`scripts/envelope.py`](scripts/envelope.py) sweeps a
producer across the whole chain and appends one row *per rung per run* to
[`docs/envelope.jsonl`](docs/envelope.jsonl) — the provenance cost envelope
across diverse models, taken at each layer.

It does — and the redo's ledger accounts what the model actually cost: the
producer reports real token/usd usage back through `$RETICULI_USAGE`, one line
per blind call. In one such run (sonnet-5), given only `kernel_check.py` and
...
(`a1fc505a`, `30968ae8`, the claim as it then stood — the checks have since
grown the cost-ledger clauses, re-minting the roots above). A different kernel
and a different toolchain, one claim. Run it again and the model writes
different code again; the roots don't move. That is the three-machine test with
an independent producer: the basin is real, not a copy.
...
- [`docs/landscape.md`](docs/landscape.md) — prior art and impact: what exists,
  what doesn't, and the limits that will decide it.
- [`docs/three_machine_problem_white_paper.pdf`](docs/three_machine_problem_white_paper.pdf)
